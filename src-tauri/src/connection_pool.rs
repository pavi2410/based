            ...
            .0
        {
            ...
            "mongodb" => {
                ...
                if let Err(ref e) = client_result {
                    ...
                    if error_msg.contains("SCRAM failure")
                ...
                let db_name = conn_url.split('/').last().ok_or_else(|| {
                    crate::Error::InvalidDbUrl(format!(
                ...
                if clean_db_name.trim().is_empty() {
                    return Err(crate::Error::InvalidDbUrl(format!(
                        "Empty database name in connection string: {}",
                        conn_url
                    )));
                }
                ...
                if clean_db_name.contains('.') {
                    return Err(crate::Error::InvalidDbUrl(format!(
                        "Invalid database name '{}': '.' is an invalid character in a db name",
                        clean_db_name
                    )));
                }
                ...
                match db.run_command(command_doc, None).await {
                    ...
                    Err(e) => {
                            ...
                            || error_msg.contains("Authentication failed")
                        {
                            ...
                            Err(crate::Error::MongoAuth(
                                "Authorization failed. User may not have access to this database."
                                    .to_string(),
                            ))
                        } else {
